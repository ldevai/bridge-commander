...
| `lieutenant.patch` | `color?, avatar?, voice?, name?, prefix?, harness?, model?, ref? → lieutenant` | 🤠 (⋯ → settings) · ⚓ (CLI) · ⚙️ (ref re-registration on init idempotency) | cosmetics + voice + the card-id `prefix` (refused when another lieutenant already holds it; already-minted ids never change — a new prefix is about what comes next); `name` changes the display only — `id` and the derived session name stay immutable; `avatar: null` clears back to the colored-dot fallback; `voice: ""`/`null` clears back to the board's voice. `model` is stored on the lieutenant and handed to the harness CLI as `--model` on every spawn AND resume of it, so a respawn keeps it — the board holds no list of model names, the string goes straight through; `null`/`""` clears it back to the harness default (`~/.codex/config.toml` for codex). `harness` MOVES a living lieutenant, which retire cannot (it is refused while cards are owned): the `lt` window is killed (never the session — its worker windows live there), `ref` is rewritten whole as `{harness, session, cwd, window: 'lt'}` with NO `resumeId` (an id one harness minted means nothing to another), and the lieutenant respawns on the same from-nothing path `/reset` and supervision take — doctrine + charter + owned cards + pending queue. Level-1 `harness-switch` event; the conversation is lost, the delivery queue is not. Same harness = no-op, no session touched; a lieutenant with no `ref` is refused (a harness is a property of the session). Applied last in the patch, so a `harness` + `model` call lands on both |
...
| lieutenant session dies | server auto-respawn (resume when possible; else relaunch with charter + owned cards + pending queue as the prompt — plus the paths of any `handoff*` artifacts on its plan cards, as "read these first" pointers), level-1 event, drain nudge; 3 failed attempts → level-1 needs-captain |
